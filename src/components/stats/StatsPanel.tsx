      ...
      habits
        ...
        .map((habit) => {
          const values = records
            .map((record) => record.statuses[habit.id])
            .filter(Boolean)
            .map((status) => statusScore[status])
            .filter((score): score is number => score !== null)
          const rate = values.length === 0 ? 1 : values.reduce((sum, value) => sum + value, 0) / values.length
          return { habit, rate }
        })
  ...
  return (
    <section className="side-page stats-page" data-testid="stats-page">
      <header className="side-header stats-header">
        ...
        <span className="flight-hours numeric">{Math.floor(flightMinutes / 60)}:{String(flightMinutes % 60).padStart(2, '0')}</span>
      </header>

      <div className="impact-hero">
        <p className="label-caps">Wenn du diesen Kurs hältst</p>
        <strong className="numeric">{missDistance} KM</strong>
        <span>neben {journey.destinationIata}</span>
        <div className="route-comparison"><i /><i style={{ transform: `rotate(${Math.min(12, deviation)}deg)` }} /></div>
      </div>
      ...
      <section className="stats-section">
        <div className="section-heading"><div><p className="label-caps-micro">Einfluss</p><h2>Deine Habits</h2></div><span>30 Tage</span></div>
        <div className="habit-rate-list">
          {habitRates.map(({ habit, rate }) => (
            <div key={habit.id} className="habit-rate-row">
              <span>{habit.icon} {habit.name}</span>
              <div><i style={{ width: `${Math.round(rate * 100)}%` }} /></div>
              <strong className="numeric">{Math.round(rate * 100)}%</strong>
            </div>
          ))}
        </div>
      </section>

      <section className="stats-section hangar-section">
        <div className="section-heading"><div><p className="label-caps-micro">Belohnung</p><h2>Hangar</h2></div><span>{activeWeeks} aktive Wochen</span></div>
        <div className="aircraft-carousel">
          {AIRCRAFT.map((aircraft) => {
            const unlocked = activeWeeks >= aircraft.requiredWeeks
            return (
              <button
                type="button"
                key={aircraft.id}
                className={selectedAircraft === aircraft.id ? 'aircraft-card selected' : 'aircraft-card'}
                disabled={!unlocked}
                onClick={() => selectAircraft(aircraft.id)}
                style={{ '--aircraft-accent': aircraft.accent } as React.CSSProperties}
              >
                <span className="aircraft-glyph">✈</span>
                <strong>{aircraft.name}</strong>
                <small>{unlocked ? aircraft.caption : `Ab Woche ${aircraft.requiredWeeks}`}</small>
              </button>
            )
          })}
        </div>
