...
export function StatsPanel({ onBackToFlight }: { onBackToFlight: () => void }) {
  ...
  return (
    <section className="side-page stats-page" data-testid="stats-page">
      ...
      <section className="stats-section hangar-section"><div className="section-heading"><div><p className="label-caps-micro">Belohnung</p><h2>Hangar</h2></div><span>{activeWeeks} aktive Wochen</span></div><div className="aircraft-carousel">{AIRCRAFT.map((aircraft) => { const unlocked = activeWeeks >= aircraft.requiredWeeks; return <button type="button" key={aircraft.id} className={selectedAircraft === aircraft.id ? 'aircraft-card selected' : 'aircraft-card'} disabled={!unlocked} onClick={() => selectAircraft(aircraft.id)} style={{ '--aircraft-accent': aircraft.accent } as React.CSSProperties}><span className="aircraft-glyph"><AircraftGlyph size={26} color={aircraft.accent} rotationDegrees={-45} /></span><strong>{aircraft.name}</strong><small>{unlocked ? aircraft.caption : `Ab Woche ${aircraft.requiredWeeks}`}</small></button> })}</div></section>
